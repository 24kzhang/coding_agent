...
- Loop/Stop：Coder 最多 6 步、`done` 字段、Verifier 最多触发一次修复回路。
...
当前项目使用双重终止：模型 `done=true` 与最多 6 步硬限制。Verifier 回路也有限制：Coder 第一次执行后 `retry` 变为 1，验证失败可再回 Coder；第二轮后不再继续循环。
...
1. 最多列出 200 个文件。
2. 优先读取 README、`pyproject.toml`、`package.json` 和常见入口。
3. 再读取前 30 个代码、配置和文档文件的片段。
4. 根据扩展名识别技术栈。

这是一个轻量 baseline，适合小仓库。它还不是语义代码检索：
...
项目目录使用规范化 workdir 的哈希作为 id，避免目录名冲突，并把真实路径保存到项目 Markdown 中。
...
系统先记录用户输入，再执行 Agent。正常结束时 Final 会追加：

```text
ag=manager, tl=final, k=result
```

如果会话最后一条不是该记录，`interrupted()` 就认为上次可能异常中断。Manager 会把提示加入 recent context，要求下游以当前磁盘状态为准。

这是一种 **日志尾标记恢复**，能发现中断，但还不是精确恢复：它不知道具体节点是否可重放，也不能保证工具操作幂等。生产化需要节点 checkpoint、action id、幂等键和操作状态机。
...
当前 `MemoryStore` 提供读写项目/全局 Markdown 的能力，但最终节点没有自动从对话中提取并更新长期偏好。这一点应如实说明。生产化可以增加 memory curator，同时要求：
...
- 截断长输出；
- 对 `rm`、`format`、`git reset` 等危险前缀做拒绝。

但它使用 `shell=True`，危险前缀规则不是完整命令解析器。以下风险仍存在：
...
- 只有 Python 文件：`python -m py_compile ...`。
- 有 `package.json`：`npm test -- --run`。
- 静态 Web：检查 DOM id 与函数接线，并记录可用 HTTP 服务打开。
- 最多运行 3 条命令，单条测试 timeout 为 240 秒。
...
第二轮 Coder 会在 observations 中看到上一轮结构化测试结果，因此能针对 stderr 修复，而不是重新盲猜。

### 13.4 验证策略的局限

- `pyproject.toml` 存在不代表一定配置了 pytest。
- `npm test -- --run` 不适用于所有 Node 项目。
- 没有按变更范围选择最相关测试。
- 没有覆盖 lint、type check、build、集成测试和端到端测试的统一策略。
- 测试通过也不能证明需求验收通过。
- Agent 可能修改测试迎合错误实现。
...
- 每次请求创建一个 daemon thread，高并发会产生大量线程。
- Queue 没有大小上限，慢客户端可能积压内存。
- 同一会话并发发送会同时追加 JSONL，缺少会话锁。
- `ModelStore` 和配置文件写入缺少事务/锁。
- 断开 HTTP 连接后，后台任务可能仍继续执行。
- 没有任务取消、超时总预算和状态查询接口。

生产化可使用 asyncio + 异步 HTTP 客户端、任务队列、每会话互斥锁、持久化 job 表、背压和取消令牌。
...
- 文件工具限制工作目录，Shell 具备基础高风险命令拦截和超时。
- 验证失败可反馈给 Coder 自动修复。
- 自定义 JSONL 会话记忆支持历史恢复、中断检测和窗口阈值压缩。
...
- 不是真正逐 token 流式输出。
- Repo Reader 不是向量数据库/RAG，只是规则扫描与片段读取。
- 记忆压缩不是 LLM 语义摘要。
- 长期记忆没有自动、安全地提取和更新。
- Shell 隔离不是容器级沙箱。
- 当前适合本地单用户，不是多租户生产系统。
...
1. **正确性**：结构化模型输出 schema、工具参数校验、Plan step 状态。
2. **安全性**：命令白名单、密钥管理、敏感文件过滤、人工确认。
3. **恢复性**：LangGraph checkpointer、action 幂等、任务状态表。
4. **检索质量**：符号索引、调用图、查询相关代码选择。
5. **并发能力**：异步客户端、任务队列、会话锁和取消。
6. **评测体系**：离线任务集、成功率、成本、延迟与回归基线。
7. **可观察性**：trace、节点耗时、成本与错误分类仪表盘。
...
每轮模型返回 `done`，这是软终止；代码还有最多 6 步的硬上限。每个 Shell 命令也有 timeout，长输出会截断。验证修复回路最多两轮 Coder。
...
不完全安全。当前黑名单能防止模型直接以 `rm`、`git reset` 等开头执行，但 `shell=True` 允许组合命令，简单前缀无法覆盖所有绕过方式。它是本地原型的第一层保护，不是生产沙箱。

生产化应改为命令白名单和参数级校验，默认 `shell=False`，在容器中限制文件系统、网络和资源，高风险动作通过人工确认。安全必须在执行层强制，不能只写 Prompt。

### Q21：为什么 Coder 写完整文件，而不是输出 diff

**参考答案：**

当前 Prompt 要求 `write_file` 提供完整内容，实现简单，避免自行解析模型生成的 patch。但大文件会消耗更多 token，也可能覆盖用户未相关修改。

更成熟的实现应支持结构化 patch，并在应用前校验上下文、展示 diff、检测脏工作区冲突。无论完整写还是 patch，都应先读当前文件并保留用户已有修改。
...
当前项目实现了第一层，并在 Prompt 中要求最小改动，但后几层仍可加强。Prompt 约束不是强安全边界。
...
正常结束时 Final 会写 `manager/final/result`。读取会话最后一行，如果不是这个终止标记，就认为可能中断。下一次 Manager 把中断提示加入上下文，并以磁盘现状继续。
...
当前按字符粗估 token，压缩保留开头、结尾和每个工具最近输出。更好的做法是用对应模型 tokenizer 精确计数，并按每个 Agent 的输入预算动态分配，而不是整个会话统一阈值。
...
当前项目提供长期记忆文件和更新方法，但没有自动抽取链路。若加入，我会先生成候选，再做规则过滤和冲突检测，重要偏好让用户确认，并支持可见、可编辑、可删除。
...
当前每个模型配置有 HTTP timeout，并将网络异常统一为 `LlmError`。但还没有完整重试和熔断。
...
当前可能创建两个线程同时追加同一 JSONL、读取相同 pending plan 并修改同一工作区，存在竞态。这是本地单用户原型的限制。

应为每个 session 或 workdir 建立互斥锁/任务队列；同一项目的写任务串行，只读任务可按策略并发；记忆写入用数据库事务或文件锁，并给每轮消息 sequence id。
...
这是成本与可靠性的保守上限，避免 Agent 在同一错误上无限循环。第一轮实现后允许一轮基于测试反馈的修复，第二次仍失败就把证据交给用户。
...
当前会进入 Verifier，可能因为已有测试通过而误判完成。应增加任务不变量：代码生成/修改任务若 `changes` 为空，需要 Coder 解释原因或视为未完成；同时对比执行前后 git diff，并用 acceptance evaluator 检查需求。
...
我会从可验证的工程行为回答：任务动态路由而非固定流水线；Plan 选项跨请求恢复并重建上下文；Coder 的工具动作真实落盘；Verifier 失败形成闭环；路径越界被应用层拒绝；中断由日志尾标记识别；模型可按 Agent 路由；关键行为有 pytest 回归测试。
