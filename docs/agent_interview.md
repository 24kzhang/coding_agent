...
当前实现对普通兼容模型使用 JSON，对 LongCat 使用其原生工具标签，再统一转换成内部 actions。工具执行前会做白名单和必填参数校验，但还没有为每种 action 定义独立 Pydantic schema。后续可用判别联合、枚举和字段级错误恢复继续增强。
...
- Loop/Stop：单次 Coder 最多 16 步、`done` 与工具成功条件、Verifier 最多形成三次 Coding 尝试。
...
当前项目使用多重终止：模型 `done=true`、本批工具全部成功、重复动作拒绝、停滞检测和最多 16 步硬限制。Verifier 回路也有限制：每次完整 Coding 后 `retry` 加一，最多进行三次 Coding 尝试。
...
后续 Coder 会看到压缩后的结构化失败清单，并重新读取错误涉及文件的当前磁盘内容，因此能针对错误修复，而不是重新盲猜或从头覆盖。内部单次 Coding 尝试有最多 16 步 ReAct 上限。
...
### 14.4 模型流式接收不等于前端 token streaming

LongCat 调用已经启用 `stream=true`，`LlmClient` 会解析 SSE chunk 并拼接 `delta.content`。这样长工具调用可以边生成边接收，不会因为等待一个超大 JSON 响应而超时。

但前端仍只看到 Agent 阶段、工具事件和节点完成后的最终消息，并没有逐 token 更新同一个气泡。要实现前端 token streaming，还需要把 delta 写入任务事件队列、为消息分配稳定 id、处理断线后的半截结构和恢复策略。二者应明确区分。
...
每轮模型返回 `done`，这是软终止；只有工具动作成功才接受完成，代码还有最多 16 步的硬上限。每个 Shell 命令也有 timeout，长输出会截断；验证修复回路最多形成三次 Coding 尝试。
...
当前支持 `write_file`、`replace_file` 和 `replace_block`。已有文件先读取；短改动用 old/new 与预期命中次数精确替换，长函数或章节用唯一开始/结束锚点替换，避免把整段旧代码再次放入模型输出；新文件或整体结构确实需要重建时才写完整内容。模型还可以调用 `git_diff` 检查结果。
...
这是成本与可靠性的有限上限，避免 Agent 在同一错误上无限循环。首次实现失败后还能进行两次基于结构化验证结果的修复；第三次仍失败就把证据交给用户。每次 Coding 尝试内部另有最多 16 步 ReAct 限制。
